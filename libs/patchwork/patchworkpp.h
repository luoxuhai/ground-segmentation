...
struct RevertCandidate {
  ...
  RevertCandidate(int _c_idx, int _s_idx, double _flatness, double _line_var,
                  Eigen::VectorXf _pc_mean, vector<PointXYZ> _ground)
...
class PatchWorkpp {
 ...
 private:
  ...
  void extract_piecewiseground(const int zone_idx, const vector<PointXYZ> &src,
                               vector<PointXYZ> &dst, vector<PointXYZ> &non_ground_dst);

  void extract_initial_seeds(const int zone_idx, const vector<PointXYZ> &p_sorted,
