...
void PatchWorkpp::extract_initial_seeds(const int zone_idx, const vector<PointXYZ> &p_sorted,
...
void PatchWorkpp::estimateGround(Eigen::MatrixX3f cloud_in) {
    ...
    for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
      for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
        ...
        if (params_.enable_RNR) {
          ...
          for (auto &p : zone[ring_idx][sector_idx]) {
            if (p.z == std::numeric_limits<float>::min())
              it++;
            else
              break;
          }
        ...
        */
        ...
        if (!is_upright) {
          ...
          patchwork::RevertCandidate candidate(concentric_idx, sector_idx, ground_flatness,
      ...
      if (!candidates.empty()) {
        if (params_.enable_TGR) {
          temporal_ground_revert(cloud_ground_, cloud_nonground_, ringwise_flatness, candidates,
                                 concentric_idx);
        } else {
  ...
  if (params_.verbose) {
    cout << "Time taken : " << time_taken_ / double(1000000)
         << "(sec) ~ "
         //  << t_flush / double(1000000)  << "(flush) + "
         << t_czm / double(1000000) << "(czm) + " << t_sort / double(1000000) << "(sort) + "
         << t_pca / double(1000000) << "(pca) + " << t_gle / double(1000000) << "(estimate)"
         << endl;
...
void PatchWorkpp::reflected_noise_removal(Eigen::MatrixX3f &cloud_in, vector<PointXYZ> &nonground) {
  ...
  // The following code works when the ring information of points are sorted with a decreasing
  // order...
  // TODO : Change codes when it is available to get the ring information of points.
  for (int i = cloud_in.rows() - 1;
       i > cloud_in.rows() - params_.pc_num_channel * params_.noise_filter_channel_num; i--) {
    if (cloud_in.row(i)(2) > -params_.sensor_height - 0.8) continue;

    // double theta = fabs(atan2(cloud_in[i].y, cloud_in[i].x));
    // if ( min(theta, M_PI-theta) > 45.0*(M_PI/180) ) cout << "outside intensity: " <<
    // cloud_in[i].intensity << endl; if ( min(theta, M_PI-theta) < 45.0*(M_PI/180) ) cout <<
    // "inside  intensity: " << cloud_in[i].intensity << endl; if ( min(theta, M_PI-theta)
    // > 45.0*(M_PI/180) && cloud_in[i].intensity > 0.1) continue;
...
void PatchWorkpp::temporal_ground_revert(vector<PointXYZ> &cloud_ground,
                                         vector<PointXYZ> &cloud_nonground,
...
// For adaptive
void PatchWorkpp::extract_piecewiseground(const int zone_idx, const vector<PointXYZ> &src,
                                          vector<PointXYZ> &dst, vector<PointXYZ> &non_ground_dst) {
  // 0. Initialization
...
void PatchWorkpp::pc2czm(const Eigen::MatrixX3f &src, std::vector<Zone> &czm) {
  ...
  for (int i = 0; i < src.rows(); i++) {
    ...
    if ((r <= max_range) && (r > min_range)) {
      ...
      // params_.num_rings_each_zone[zone_idx] - 1); int sector_idx = min(static_cast<int>((theta /
      // sector_sizes_[zone_idx])), params_.num_sectors_each_zone[zone_idx] - 1);
